...
function renderModeBar() {
    ...
    if (modeEl) {
        ...
        modeEl.style.background = isShip
            ? 'rgba(255,255,255,0.15)'
            : 'rgba(167,139,250,0.35)';
    }
    if (switchBtn && currentUser?.role === 'admin') {
        switchBtn.style.display = 'inline-block';
        switchBtn.textContent = isShip
            ? 'Switch to DORB-Control'
            : 'Switch to DORB-Ship';
    }
}
...
function showModeSwitcher() {
    ...
    const warning = isShip
        ? 'Switching to DORB-Control connects to the fleet management database. Shore office use only.'
...
async function loadDashboard() {
    const uRes = await req('GET', '/users/');
...
const TANK_LABELS = {
    fuel_oil: 'Heavy Fuel Oil', diesel_oil: 'Diesel / MGO',
...
function renderTankGauges(containerId, tanks, selectable = false) {
    ...
    TYPE_ORDER.forEach(type => {
        ...
        grp.forEach(t => {
            ...
            const sel = selectable ? `onclick="selectTank('${t.id}','${t.name}','${t.tank_type}')"` : '';
            html += `<div class="tank-card" id="tc-${t.id}" ${sel}>
...
function selectTank(id, name, type) {
    ...
    const p1 = document.getElementById('p1-tank');
    const p2 = document.getElementById('p2-tank');
    if (p1) p1.value = id;
    if (p2) p2.value = id;
    // Show only valid codes for this tank type
    highlightValidCodes(type);
}
...
const PART1_CODES = [
    ...
    { code: 'E', label: 'Discharge of bilge water',        tanks: ['bilge', 'slop'],             desc: 'Discharge overboard or to reception facility. State position, OWS rate, PPM reading, total quantity.' },
    { code: 'F', label: 'Condition of OWS / ODM',          tanks: [],                            desc: 'Condition of oil filtering equipment. State any malfunction or repairs to OWS/ODM. Not tank-specific.' },
    { code: 'G', label: 'Accidental / other discharge',    tanks: ['__any__'],                   desc: 'Accidental or exceptional discharge. State time, position, estimated quantity, circumstances, action taken.' },
    { code: 'H', label: 'Bunkering of fuel / LO',          tanks: ['fuel_oil', 'diesel_oil', 'lubricating_oil'], desc: 'Bunkering of fuel oil or bulk lubricating oil. State port, tank, grade, quantity received (m³ and MT).' },
...
const PART2_CODES = [
    ...
    { code: 'C', label: 'Unloading of oil cargo',          tanks: ['cargo'],                     desc: 'Unloading of oil cargo. State port, tank identity, quantity discharged, quantity remaining.' },
    { code: 'D', label: 'Ballasting of cargo tanks',       tanks: ['cargo', 'ballast'],          desc: 'Ballasting of cargo tanks or dedicated clean ballast tanks. State tank, position, quantity (m³).' },
    { code: 'E', label: 'Cleaning of cargo tanks',         tanks: ['cargo'],                     desc: 'Tank cleaning including crude oil washing. State tanks, method, quantity of cleaning water.' },
    ...
    { code: 'G', label: 'Accidental / other discharge',    tanks: ['__any__'],                   desc: 'Accidental discharge. State time, position, quantity, type of oil, circumstances, action taken.' },
];

function highlightValidCodes(tankType) {
    // Highlight valid Part I codes for the selected tank
    document.querySelectorAll('#part1-codes .op-code').forEach(el => {
        const code = el.dataset.code;
        const found = PART1_CODES.find(c => c.code === code);
        if (!found) return;
        const valid = found.tanks.includes(tankType) || found.tanks.includes('__any__') || found.tanks.length === 0;
        el.style.opacity = valid ? '1' : '0.3';
        el.title = valid ? '' : `Code ${code} not applicable for ${TANK_LABELS[tankType] || tankType} tanks`;
    });
    // Same for Part II
    document.querySelectorAll('#part2-codes .op-code').forEach(el => {
        const code = el.dataset.code;
        const found = PART2_CODES.find(c => c.code === code);
        if (!found) return;
        const valid = found.tanks.includes(tankType) || found.tanks.includes('__any__');
        el.style.opacity = valid ? '1' : '0.3';
    });
}
...
function renderOpCodes(containerId, codes, part) {
    ...
    el.innerHTML = codes.map(c => `
        <div class="op-code" id="${part}-code-${c.code}" data-code="${c.code}" onclick="selectCode('${part}','${c.code}')">
...
function selectCode(part, code) {
    ...
    // Show applicable tanks in the filter
    if (found.tanks.length > 0 && !found.tanks.includes('__any__')) {
        const filter = document.getElementById('ops-tank-filter');
        if (filter && found.tanks.length === 1) {
            filter.value = found.tanks[0];
            filterOpsTanks();
        }
    }
}
...
function updateOrbModePreview(type) {
    ...
    if (el) el.value = tankerTypes.includes(type)
...
async function saveVessel(vesselId) {
    ...
    if (res && res.ok) {
        ...
        await loadVessel();
        setTimeout(() => loadSetupVessel(), 1000);
    } else {
        const err = await res?.json();
...
async function saveUser(userId) {
    ...
    if (userId) {
        const a = document.getElementById('u-active');
        if (a) data.is_active = a.value === 'true';
    } else {
...
async function loadAuditLog() {
    document.getElementById('audit-entries').innerHTML =
        '<tr><td colspan="5" class="empty-row">Audit log viewer coming in Phase 3 — all actions are recorded in the database.</td></tr>';
}
...
function showModalAlert(msg) {
    ...
    if (!a) {
        ...
        document.getElementById('modal-box').prepend(a);
    }
    a.textContent = msg;
}
...
// Load app info on page load for mode display
loadAppInfo();
